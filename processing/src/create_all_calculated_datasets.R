...
define_dataset = function(list, columnDefinition, name, annotations) {
  list[[name]] <- create_datasets(read.csv2(paste("./data/51000-0014_", name, ".csv", sep=""), na.strings = "0"), columnDefinition, name, annotations)
  return(list)
}

define_annotation = function(xmin, xmax, alpha){
  return(list(xmin=xmin, xmax=xmax, color=main_season, alpha=.15+(alpha-1)*((.45-.15)/3)))
}
...
create_all_datasets <- function(columnDefinition) {
  ...
  returnval = define_dataset(returnval, columnDefinition, "Himbeeren", list(define_annotation(7,7,4),define_annotation(6,6,2),define_annotation(8,9,2),define_annotation(5,5,1),define_annotation(10,11,1)))
  ...
  returnval = define_dataset(returnval, columnDefinition, "K\u00FCrbissen", list(define_annotation(9,9,4),define_annotation(10,10,3),define_annotation(8,8,2),define_annotation(7,7,1),define_annotation(11,12,1)))
  returnval = define_dataset(returnval, columnDefinition, "Orangen", list())
  returnval = define_dataset(returnval, columnDefinition, "Spargel", list(define_annotation(5,5,4),define_annotation(6,6,3),define_annotation(4,4,2),define_annotation(3,3,1),define_annotation(7,7,1)))
